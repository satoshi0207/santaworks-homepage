      ...
      <div className="wrap">
        <article>
          ...
          <section className="blk">
            ...
            <p>
              {/* 🔴 「1940年から続けている」を外した（2026-08-06 の検算）。
                  **一次資料に無い。**引いている報告書にも、社人研の当該ページにも
                  第1回の実施年は書かれていない（史実としては第1回=1940年だが、
                  **この記事は「出典の一次資料から直接転記」と宣言している**）。
                  ⚠️ 系列の範囲は次の段落で「第7回（1977年）から第16回（2021年）」と
                     書いており、そちらは注に逐語で載っている。 */}
              国立社会保障・人口問題研究所が続けている調査があります
          ...
          {/* 05
              🔴 キッカー「残っているところ」は**動かさない。**
                 §06「もう始まっているところ」と対（見合いはまだある／次はもう出ている）。
              🔴 題名は「見合いが、まだ残っているところ」から差し替えた
                 （Satoshiさん・2026-08-06）。**キッカーと同語反復だった。**
                 ⚠️ 26.9% を題名に出さない——この節は⚠️で「FIG 01 の 9.8% と
                    並べないでください」と断っている。**見出しはいちばん飛ばし読み
                    される場所**なので、断りより先に数字だけ持って帰られる。
                    （KPIから 74.8% を外したのと同じ判断） */}
          <section className="blk">
            <span className="kicker">残っているところ</span>
            <div className="h2">
              <span className="idx">05</span>
              <h2 className="tt">見合いは、無くなっていませんでした</h2>
            </div>
          ...
          <section className="blk">
            ...
            {/* 🔴 「時代劇の中にあるような、遠いものとして。」を削除
                （Satoshiさん・2026-08-06）。**余計な脚色。**
                この記事が書けるのは「昔の話だと思っていた」という**認識の事実**まで。
                どんなふうに遠いと感じていたかは**あとから足した絵**で、
                次の段落（60年前のことでした）が数字で覆すのはその絵ではない。 */}
            <p>調べる前、私は見合い結婚を「昔の話」だと思っていました。</p>
            <p>
            ...
            <p>
              もうひとつ、調査の分かれ方が引っかかりました。
              結婚相談所はオンラインでも「見合い」で、マッチングアプリは「ネットで」。
              <b>やっていることは近いのに、名前が違うだけで別の枠に入ります。</b>
              けれど<strong>私自身、この二つを同じものとして考えていませんでした。</strong>
              <b>なぜ分けていたのかは、説明できません。</b>
            </p>
            ...
            {/* 🔴 独立した段落だった「近いはずだ、と書きました。けれど…」を
                **上の段落に畳んだ**（Satoshiさん・2026-08-06）。
                冒頭の「近いはずだ、と書きました」が**直前の一文の言い直し**で、
                そのぶん段落ぜんぶが余分に見えていた。→ 言い直しだけ落とす。
                🔴 **中身は消せない。**§08 の締めが
                   「**私が勝手に分けていた**ものが、書類の名前として書かれている」で、
                   *私も分けていた*と言っているのはここだけ。消すと §08 が宙に浮く。 */}
          ...
          <section className="blk">
            ...
            {/* 🔴 「入会に要るものが決まっています（書類5点＋面談）」の段落を
                **削除した**（Satoshiさん・2026-08-06）。**この記事の話題ではない。**
                ⚠️ もともとの狙いは「私が感じていた線は、制度としても実装されている」
                   だったが、それを言うには**アプリ側に同じ入口が無い**ことが要る。
                   **確かめていないので書けない**（「民間は公表していない」と
                   書かないと決めたのと同じ理由）。→ **狙いごと外す。**
                🔴 残す事実は**開始日**。これは原典に書いてあり、
                   記事の題名（当たり前になってから、まだ60年）と同じことを言う。 */}
            <p>
              ⚠️ <b>申込と登録の差が何なのかは、わかりません。</b>
              途中でやめたのか、いま手続きの最中なのか、別の理由なのか、
              公表されている数字では分けられません。
              <b>入口で数が半分以下になっている</b>、というところまでです。
            </p>
            {/* 🔴 締めを差し替えた（Satoshiさん・2026-08-06）。元は
                「私が勝手に分けていたものが、**書類の名前として実際に書かれている**」。
                ⚠️ **事実として成り立っていなかった。**書類の名前は「独身証明書」
                   「誓約書」であって、**私の分け方はどこにも書かれていない。**
                続く「窓口の名前は、誰かが決めています」も、§05 の結論の言い直しで
                **この節の数字からは一歩も出ていない**（深く聞こえるだけの一文）。
                → **この節が新しく出せる事実は、開始日ひとつ。**
                🔴 §02（どの5年かは特定できない）と対にして置く。
                   ⚠️ **本文に「§02」とは書かない**（Satoshiさん・2026-08-06）。
                      **画面に「§」は一度も出ていない**（節の札は `02` という数字だけ）。
                      → 記号で指さず、**その文で中身を言い直す。**
                   **過ぎたことの始まりは数えられず、いま始まったものには日付がある。**
                ⚠️ 「都が取り組むくらいだからトレンドが変わってきている」とは書かない
                   （2026-08-06 に案が出た）。**図は累計4つで時間変化を持たない**うえ、
                   なぜ都が取り組むのかを考えさせると**少子化の枠が入る。**
                   トレンドの話は §04・§06 が数字で済ませている。 */}
            <p>
              事業の是非は、この記事では扱いません。
              ただ、ひとつ書けることがありました。
              <b>
                恋愛結婚が見合いを抜いた5年間は、特定できませんでした。
              </b>
              けれど<strong>この窓口が始まった日は、分かります</strong>
              ——令和6年9月20日<sup>※5</sup>。
              <b>いま始まったものには、日付が残ります。</b>
            </p>
          </section>
          ...
          <div className="sources">
            ...
            <ol>
              <li>
                ...
                {/* 🔴 「報告書は2022年9月公表」は誤りだった（2026-08-06 の検算で発見）。
                    **2022年9月9日は「結果の概要」**の公表日。
                    ここで引いている報告書（JNFS16_Report04.pdf）は**2023年8月公表**で、
                    PDF の作成日も 2023-08-29。社人研のページでも両者は別に載っている。 */}
                調査は2021年6月実施、報告書は2023年8月公表。
